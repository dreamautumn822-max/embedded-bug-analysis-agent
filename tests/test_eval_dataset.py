...
ALLOWED_BUG_TYPES = {"network_dhcp", "network_pppoe", "wifi_disconnect"}
REQUIRED_FIELDS = {
    "case_id",
    "device_model",
    "firmware_version",
...
def test_eval_dataset_has_three_complete_cases():
    cases = json.loads(EVAL_CASES_PATH.read_text(encoding="utf-8"))

    assert isinstance(cases, list)
    assert len(cases) == 3
    assert {case["case_id"] for case in cases} == {"EVAL-001", "EVAL-002", "EVAL-003"}
